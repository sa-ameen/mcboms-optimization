...
This module contains ONLY the data published in Harwood et al. (2003)
Tables 2 and 3. No invented or estimated values.

Key insight: We only know the costs/benefits for alternatives that were
SELECTED in either the $50M or $10M scenarios. Each site has:
  - Alternative 0: Do nothing (always available)
  - Alternative 1: The improvement selected at $50M budget
  - Alternative 2: (Site 8 only) Different improvement selected at $10M
...
def get_harwood_alternatives() -> pd.DataFrame:
    ...
    Returns DataFrame with columns:
        ...
        safety_benefit, ops_benefit, total_cost, total_benefit, net_benefit
    """
    ...
    # Alt 1: Resurface only (from Table 2)
    alternatives.append({
        "site_id": 1, "alt_id": 1, 
    ...
    # Total benefit = safety benefit + operations benefit
    df["total_benefit"] = df["safety_benefit"] + df["ops_benefit"]
    
    # Net benefit per Harwood = total_benefit - safety_improvement_cost
    # (resurfacing cost is NOT subtracted - it's only in budget constraint)
